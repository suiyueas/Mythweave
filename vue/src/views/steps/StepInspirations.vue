<template>
  <div class="step-inspirations">
    <div class="step-header">
      <h2>💡 灵感素材</h2>
      <p class="step-desc">AI 将根据完整设定生成写作灵感素材库</p>
    </div>
    ...
    <div v-else-if="status === 'generating'" class="step-body generating">
      <div class="generating-status">
        ...
        <span>AI 正在生成灵感素材库...</span>
      </div>
      <div class="generating-log">
        <div class="log-item"><span class="log-icon">💬</span><span>生成对白灵感</span></div>
        <div class="log-item" style="animation-delay:2s"><span class="log-icon">🎬</span><span>生成场景描写素材</span></div>
        <div class="log-item" style="animation-delay:4s"><span class="log-icon">📚</span><span>生成典故隐喻与人物细节</span></div>
      </div>
    </div>

    <div v-else-if="status === 'completed'" class="step-body completed">
      <div class="result-summary">
        <span class="badge success">✅ 共 {{ preview.count || '?' }} 条灵感素材</span>
      </div>
      <div class="finish-note">
        <p>🎉 所有设定已生成完毕！你可以进入写作工坊开始创作了。</p>
      </div>
      <div class="step-actions">
        ...
        <button class="btn-primary" @click="$emit('complete')">✍️ 开始写作 →</button>
      </div>
    </div>
    ...
    <div v-else-if="status === 'skipped'" class="step-body skipped">
      ...
      <div class="finish-note"><p>🎉 所有步骤已完成！你可以进入写作工坊开始创作了。</p></div>
      <div class="step-actions">
...
const emit = defineEmits(['generated', 'skipped', 'complete', 'prev'])
...
async function generate() {
  if (!requireVip('灵感素材生成')) return
  loading.value = true; status.value = 'generating'; errorMsg.value = ''
  try {
    const res = await setupApi.generateInspirations(props.projectId, {
...
function skip() { emit('skipped'); emit('complete') }
</script>
...
.finish-note { margin-bottom: 20px; padding: 16px; background: #fefce8; border-radius: 12px; border: 1px solid #fef08a; }
.finish-note p { margin: 0; font-size: 14px; color: #a16207; line-height: 1.5; }
